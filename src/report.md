...
![ubuntu_ver](1_ubuntu_ver.png "Вывод версии Ubuntu")

## Part 2. Создание пользователя
- Создаем пользователя

    ![useradd](2_useradd.png "Создание пользователя")
- Выводим список пользователей

    ![cat_user](2_cat_user.png "Список пользователей")

- Добавлем пользователя в группу

    ![group_adm](2_group_adm.png "Добавление пользователя в группу")
...
1. **Смена названия машины на user-1**
    - С помощью vim меняем содержимое файла /etc/hostname
        ...
        ![change_host](3_1_change_host.png "Меняем содержимое файла с хостом")
    - Выводим hostname

        ![cat_hostname](3_1_cat_hostname.png "Создание пользователя")
    - Выводим hostname после перезагрузки

        ![hostname](3_1_hostname.png "Создание пользователя")
    
2. **Установка временной зоны, соответствующей вашему текущему местоположению.**
    - Текущая временная зона

        ![timedatectl](3_2_timedatectl.png "Временная зона")
    ...
    - Меняем временную зону
        ...
        ![change_timezome](3_2_change_timezome.png "Изменение часового пояса")
    - Проверяем

        ![timedatectl_new](3_2_timedatectl_new.png "Временная зона")
...
3. **Вывод названия сетевых интерфейсов с помощью консольной команды.**
    ...
    - С помощью команды `ip a` (или `ip address show`) выыедем названия сетевых интерфейсов
        ...
        ![ip_show](3_3_ip_show.png "Сетевые интерфейсы")

    - Или в укороченном виде

        ![ip_show_short](3_3_ip_show_short.png "Сетевые интерфейсы")
...
4. **Вывод ip адрес устройства от DHCP сервера  помощью консольной команды.**
    ...
    - Для просмотра всех записей в таблице маршрутизации используем команду `ip route` (или `ip r`):
        ...
        ![ip_route](3_4_ip_route.png "ip адрес устройства от DHCP сервера")
...
6. **Статичные (заданные вручную, а не полученные от DHCP сервера) настройки ip, gw, dns (использовать публичный DNS серверы, например 1.1.1.1 или 8.8.8.8).**
    ...
    Его изначальный вид \
    ![00_installer_config_0](3_6_00_installer_config_0.png "Файл конфигурации Netplan")
    После изменений \
    ![00_installer_config_new](3_6_00_installer_config_new.png "Файл конфигурации Netplan")
...
7. **Перезагрузка виртуальной машины. Убедиться, что статичные сетевые настройки (ip, gw, dns) соответствуют заданным в предыдущем пункте.**
    ...
    ![check_ip_1](3_7_check_ip_1.png "Внутренний IP после перезагрузки")
    ![check_ip_2](3_7_check_ip_2.png "Внешний IP после перезагрузки")
    ![check_ip_dns](3_7_check_dns.png "DNS после перезагрузки")

    - Выполняем пинг удаленных хостов 1.1.1.1 и ya.ru \
    ![ping_1](3_7_ping_1.png "Пинг 1.1.1.1")
    ![ping_ya](3_7_ping_ya.png "Пинг ya.ru")

## Part 4. Обновление ОС
- Для получения списка новых пактов, которые можно установить, выполняем команду `sudo apt-get update`:\
![update](4_update.png "Список обновлений")
...
![upgrade](4_upgrade.png "Обновления отсутствуют")
...
![sudo_cat](5_sudo_cat.png "Даём права sudo для Cat")
![su_cat](5_su_cat.png "Переключаемся на Cat") 
\
Как и в Part 3, в файле /etc/hostname меняем hostname на *flaffy*, перезагружаем машину и проверяем hostname \
![new_host](5_new_host.png "Даём права sudo для Cat")
...
2. **Создание текстовых файлов**
    ...
    ![test_nano](7_2_test_nano.png "nano")
    ...
    ![test_mcedit](7_2_test_mcedit.png "mcedit")
...
4. **Поиск по содержимому файла и замена слов**
    ...
    ![replace_nano_1](7_4_replace_nano_1.png "nano") \
    ![replace_nano_2](7_4_replace_nano_2.png "nano") \
    ![replace_nano_2](7_4_replace_nano_3.png "nano") 

    - Для поиска в *mcedit* нажимаем `fn+F7`, вводим слово и параметры. \
    ![find_mcedit](7_4_find_mcedit.png "mcedit") \
    ![find_mcedit_2](7_4_find_mcedit_2.png "mcedit") \
    Для замены нажимаем `fn+F4`, вводим слово и параметры. \
    ![replace_mcedit](7_4_replace_mcedit.png "mcedit") \
    ![replace_mcedit_1](7_4_replace_mcedit_1.png "mcedit") \
    ![replace_mcedit_2](7_4_replace_mcedit_2.png "mcedit")
...
![netstat](8_5_netstat.png "netstat")
...
2. Команда `htop`:
    ...
    Сортировка по:
        - PID \
        ![htop_PID](9_2_htop_PID.png "PID")
        - PERCENT_CPU \
        ![htop_CPU](9_2_htop_CPU.png "CPU")
        - PERCENT_MEM \
        ![htop_PERCENT_MEM](9_2_htop_PERCENT_MEM.png "PERCENT_MEM")
        - TIME \
        ![htop_TIME](9_2_htop_TIME.png "TIME")
    - Для фильтрации для процесса *sshd* нажимаем `fn+F4` и вводим "sshd" \
        ![sshd](9_2_sshd.png "sshd")
    - Для поиска *syslog* нажимаем `fn+F3` и вводим "syslog" \
        ![syslog](9_2_syslog.png "syslog")
...
![fdisk](10_fdisk.png "fdisk")
Для просмотра *swap* используем `swapon -s` \
![swap](10_swap.png "swap")
...
1. Команда `df`: <!-- ![df](11_df.png "df") --> \
![df](11_df_root.png "df")
...
2. Команда `df -Th`: <!-- ![df_Th](11_df_Th.png "df -Th") --> \
![df_Th](11_df_Th_root.png "df -Th")
...
2. Вывод папок:
    - /home \
        ![home](12_2_home.png "home")
    - /var \
        ![var](12_2_var.png "var")
    - /var/log \
        ![var_log](12_2_var_log.png "var/log")
3. Вывести размер всего содержимого в /var/log \
    ![var_log](12_3_var_log.png "var/log")

## Part 13. Установка и использование утилиты **ncdu**
1. Установливаем утилиту *ncdu* командой `sudo apt-get install ncdu` \
    ![ncdu](13_1_ncdu.png "Установка ncdu")
2. Выводим размер папок:
    - /home \
        ![home](13_2_home.png "home")
    - /var \
        ![var](13_2_var.png "var")
    - /var/log \
        ![var_log](13_2_var_log.png "var/log")
